...
export default function Navbar({ scrollToSection }: NavbarProps) {
  ...
  return (
    <header className="bg-white border-b shadow border-gray-100">
      <nav className="flex items-center justify-between p-6 lg:px-8" aria-label="Global">
        <div className="flex lg:flex-1">
          <a href="/" className="-m-1.5 p-1.5">
            <span className="font-bold text-xl">SCT Demo LP</span>
            ...
            <div className="-my-6 divide-y divide-gray-500/10">
              <div className="space-y-2 py-6">
                {navigation.map((item) => (
                  <button
                    key={item.name}
                    className="-mx-3 block rounded-lg px-3 py-2 text-base font-normal tracking-wider leading-7 text-gray-900 hover:bg-gray-50"
                    onClick={() => handleNavigationClick(item.href.slice(1))}
                    >
                    {item.name}
                  </button>
